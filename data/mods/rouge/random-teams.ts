...
type MoveEnforcementChecker = (
	...
	isLead: boolean, isDoubles: boolean, teraType: string, 
) => boolean;
...
export class RandomTeams {
	...
	sampleNoReplace(list: any[]) {
		...
		return RandomTeams.fastPop(list, index);
	}
	...
	cullMovePool(
		...
		if (moves.size === this.maxMoveCount - 2) {
			...
			for (const pair of MovePairs) {
				if (movePool.includes(pair[0]) && movePool.includes(pair[1])) {
					RandomTeams.fastPop(unpairedMoves, unpairedMoves.indexOf(pair[0]));
		...
		if (moves.size === this.maxMoveCount - 1) {
			for (const pair of MovePairs) {
				if (movePool.includes(pair[0]) && movePool.includes(pair[1])) {
					RandomTeams.fastPop(movePool, movePool.indexOf(pair[0]));
					RandomTeams.fastPop(movePool, movePool.indexOf(pair[1]));
				}
			}
		}
		...
		if (!teamDetails.defog && !teamDetails.rapidSpin && species.id === 'cryogonal') {
			if (movePool.includes('haze')) RandomTeams.fastPop(movePool, movePool.indexOf('haze'));
		}
		// Magnezone
			...
			if (moveArrayB.includes(moveid1)) {
				for (const moveid2 of moveArrayA) {
					if (moveid1 !== moveid2 && movePool.includes(moveid2)) {
						RandomTeams.fastPop(movePool, movePool.indexOf(moveid2));
						if (moves.size + movePool.length <= this.maxMoveCount) return;
					}
				}
			}
	...
	addMove(
		...
		this.cullMovePool(types, moves, abilities, counter, movePool, teamDetails, species, isLead, isDoubles, teraType );
		return counter;
	}
	...
	getMoveType(move: Move, species: Species, abilities: Set<string>, teraType: string): string {
		...
		if (['judgment', 'revelationdance','multiattack'].includes(move.id)) return species.types[0];
	...
	randomMoveset(
		...
		//this.cullMovePool(types, moves, abilities, counter, movePool, teamDetails, species, isLead, isDoubles, teraType);
		const itemin=this.dex.items.get(item)
		...
		const runEnforcementChecker = (checkerName: string) => {
			...
			return this.moveEnforcementCheckers[checkerName](
				movePool, moves, abilities, types, counter, species, teamDetails, isLead, isDoubles, teraType
			);
		};
		if(['Assault Vest', 'Choice Scarf', 'Choice Specs','Choice Band'].includes(itemin.name)){
			movePool=movePool.filter(x=>this.dex.moves.get(x).category!=='Status')
		}
		if (movePool.length <= this.maxMoveCount) {
			for (const moveid of movePool) {
				moves.add(moveid);
		...
		// if (species.requiredMove &&this.randomChance(1,2)) {
		...
		if (movePool.includes('facade') && ['Flame Orb','Toxic Orb'].includes(itemin.name)&&this.randomChance(1,2)) {
		...
		// if ( priorityPokemon.includes(species.id)) {
		// 	const priorityMoves = [];
		// 	for (const moveid of movePool) {
		// 		const move = this.dex.moves.get(moveid);
		// 		const moveType = this.getMoveType(move, species, abilities, teraType);
		...
		for (const type of types) {
			...
			while (runEnforcementChecker(type)) {
				if (!stabMoves.length||this.randomChance(1,5)) break;
		...
		if (!counter.stabCounter &&this.randomChance(4,5)&&moves.size < this.maxMoveCount) {
			const stabMoves = [];
			for (const moveid of movePool) {
				const move = this.dex.moves.get(moveid);
				const moveType = this.getMoveType(move, species, abilities, teraType);
		...
		// Enforce recovery
		if (this.randomChance(3,5)&&moves.size < this.maxMoveCount) {
		...
		if (this.randomChance(4,5)&&moves.size < this.maxMoveCount) {
	...
	randomSet(
		...
		// Get moves
		const moves = this.randomMoveset(types, abilities, teamDetails, species, isLead, isDoubles, movePool, teraType,'1');
		const counter = this.queryMoves(moves, species, teraType, abilities);

		// Get ability
		ability = this.getAbility(types, moves, abilities, counter, teamDetails, species, isLead, isDoubles, teraType,'1');

		// Get items
		// First, the priority items
		item = this.getPriorityItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles, teraType,'1');
		if (item === undefined && isDoubles) {
			item = this.getDoublesItem(ability, types, moves, counter, teamDetails, species, teraType,'1');
		}
		if (item === undefined) {
			item = this.getItem(ability, types, moves, counter, teamDetails, species, isLead, isDoubles, teraType,'1');
		}
